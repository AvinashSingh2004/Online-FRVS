...
def cast_vote():
    ...
    # Handle vote casting logic here
    ...
    try:
        ...
        if output is not None:
            vote_status = check_if_exists(output[0], aadhar, room_id)  # Check if this voter already voted
            if vote_status == "already_voted":
                speak("YOU HAVE ALREADY VOTED")
                flash("You have already voted")
                return redirect(url_for('index'))
            elif vote_status == "new_room":
                # Create a new file for the new room
...
def add_faces():
    ...
    upload_to_drive(faces_data_pickle, file_name, folder_id, room_id)

 # Check existence in Drive
    if check_file_exists_in_drive(file_name, folder_id):
